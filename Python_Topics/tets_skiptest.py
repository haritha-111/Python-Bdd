...
#@pytest.mark.skipif(sys.version_info > (3,9), reason == "doesn't work on py version above 3.6" )
#@pytest.mark.skipif(cent_to_fah()== 32, reason == "default value test , so skipping")
def test_case02():
    assert cent_to_fah() == 32


@pytest.mark.skipif(pytest.__version__ < '8.2.2', reason == 'pytest version is less')
def test_case03():
